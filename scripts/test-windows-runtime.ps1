...
try {
  ...
  if (-not $inputOk) {
    $details = Get-CrashDetails
    throw ("Radio Balkan did not complete the full-catalog mouse-click responsiveness smoke (navigation, supplemental filters, rendered station Play, post-load repaint, and volume while the audio backend lock was held).`n{0}" -f $details)
  }
