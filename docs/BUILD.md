...
```bash
python scripts/bump_version.py 0.0.45
```
...
Publish workflow se pokreće tek nakon uspješnog `CI` workflowa na `main`, ponovno gradi Windows, browser i Android artefakte, provjerava clean-worktree invariant, generira deterministički SHA-256 manifest koji ne može uključiti samoga sebe i odmah ga validira s `sha256sum -c` prije GitHub Release uploada. Nova verzija objavljuje se samo kada za trenutačni `VERSION` još ne postoji tag na drugom commitu; tag/SHA guard odbija objavu iste verzije s različitog SHA-a. Ručni `workflow_dispatch` namjerno nije dio produkcijskog Publish puta. Browser screenshot runtime nalazi se izvan `extensions/` i koristi se samo u privremenoj preview kopiji tijekom `Product screenshots` workflowa; produkcijski browser ZIP-ovi ga ne sadrže.
